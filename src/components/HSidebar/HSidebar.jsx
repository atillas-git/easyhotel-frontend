            ...
            return(
              <ListItem key={sidebarItem.key} sx={styles.listItem(theme)}>
                <ListItemButton sx={styles.listItemButton()}>
                  <Accordion expanded={expanded === `panel${index+1}`} onChange={handleChange(`panel${index+1}`)} sx={styles.accordion(theme)}>
                    <AccordionSummary
                      expandIcon={<ExpandMoreIcon />}
                      aria-controls={`panel${index+1}bh-content`}
                      id={`panel${index+1}bh-header`}
                      sx={{m:0,p:0,"&:hover":{color:theme.palette.primary.dark}}}
                    >
